...
int main()
{
	...
	leftImg = imread("/home/pi/Desktop/CS700_RP/stereo_dataset/city/left/0000000044.png", IMREAD_ANYDEPTH);
	rightImg = imread("/home/pi/Desktop/CS700_RP/stereo_dataset/city/right/0000000044.png", IMREAD_ANYDEPTH);
					
	Mat disp8;
	
	Mat depthMap = Mat(leftImg.size().height, leftImg.size().width, CV_16S);
	
	Ptr<StereoSGBM> sbm  = StereoSGBM::create(0,max_disp,wsize);

	sbm->compute(leftImg, rightImg, depthMap);		
	
	normalize(depthMap, disp8, 0, 255, CV_MINMAX, CV_8U);
		
	//~ imshow("TEST", disp8);
	
	//~ waitKey(0);
	imwrite("disparity_SGBM.png", depthMap);
